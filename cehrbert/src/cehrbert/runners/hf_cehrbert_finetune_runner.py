    ...
    if processed_dataset is None:
        if is_main_process(training_args.local_rank):
            if cehrbert_args.tokenized_full_dataset_path is not None:
                ...
                # Predict-only runs (do_predict without do_train) operate on a standalone
                # cohort -- e.g. a held-out test set split off up front. extract_cohort_sequences
                # preserves the source train/validation splits, so collapse everything into a
                # single 'test' split that the do_predict block below can consume.
                if training_args.do_predict and not training_args.do_train:
                    processed_dataset = DatasetDict(
                        {"test": concatenate_datasets(list(processed_dataset.values()))}
                    )
            else:
                # Organize them into a single DatasetDict
