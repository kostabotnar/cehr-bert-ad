...
def convert_index_date_to_utc_timestamp(index_date: Union[datetime.date, datetime.datetime]) -> float:
    """
    Convert a date or datetime to a POSIX timestamp interpreted as UTC.

    This mirrors how ``epoch_times`` are constructed (event times are converted with
    ``.replace(tzinfo=datetime.timezone.utc).timestamp()``), so prediction windows and
    event times are compared on the same time base. Unlike ``datetime.datetime``,
    ``datetime.date`` does not accept a ``tzinfo`` argument in ``replace``, so dates are
    promoted to midnight datetimes first.

    Parameters
    ----------
    index_date : Union[datetime.date, datetime.datetime]
        The date or datetime object to convert.

    Returns
    -------
    float
        The POSIX time in seconds (UTC) as a float.
    """
    ...
    if isinstance(index_date, datetime.datetime):
        ...
        return datetime.datetime.combine(
            index_date, datetime.datetime.min.time(), tzinfo=datetime.timezone.utc
        ).timestamp()
    else:
        raise ValueError("index_date must be datetime.date or datetime.datetime")
...
class MedToCehrBertDatasetMapping(DatasetMapping):
    ...
    def transform(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if record.get("index_date", None) is not None:
            cehrbert_record["index_date"] = convert_index_date_to_utc_timestamp(record["index_date"])
        if "label" in record:
...
class ExtractTokenizedSequenceDataMapping:
    ...
    def transform(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # index_date is normalized to a UTC POSIX timestamp (float) upstream when
        # person_index_date_map is built (see runners/data_utils.py), so we compare floats here.
        prediction_start_end_times = [
            (
                self._calculate_prediction_start_time(prediction_time_label_map["index_date"]),
