...
class CehrBertDataCollator:
    ...
    def generate_start_end_index(
        ...
        if self.truncate_type == TruncationType.RANDOM_TRUNCATION:
            ...
            # We randomly pick a [VS] token within the window of valid start positions so a
            # full-length sample can be extracted starting at a visit boundary.
            starting_points = []
            ...
            if starting_points:
                ...
                # No [VS] token exists in the valid-start window. This happens when the sequence
                # only slightly exceeds the max length and the leading tokens fall mid-visit
                # (e.g. a 513-516 token sequence with new_max_length=511). Fall back to the
                # earliest [VS] token at/after the window so the sample still begins at a visit
                # boundary, even if it ends up shorter than new_max_length. As a last resort
                # (no [VS] token anywhere in the tail), keep the most recent new_max_length tokens.
                start_index = next(
                    (
                        i
                        for i in range(seq_length - new_max_length, seq_length)
                        if record["input_ids"][i] == self.vs_token_id
                    ),
                    seq_length - new_max_length,
                )
